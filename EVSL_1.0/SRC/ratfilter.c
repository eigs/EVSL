...
void weights(int n, complex double* zk, int* mulp, double lambda, complex double* omega){
  ...
  rhs = (complex double*)malloc(m*sizeof(complex double));
  A = (complex double*)malloc(nf*nf*sizeof(complex double));
  B = (complex double*)malloc(nf*nf*sizeof(complex double));
  mat = (complex double*)malloc(4*nf*nf*sizeof(complex double));
  for(i=0;i<nf;i++){
    for(j=0;j<nf;j++){
      A[i*nf+j] = 0.0+0.0*I;
      B[i*nf+j] = 0.0+0.0*I;
  ...
  for(ii=0;ii<n;ii++){
	...
	for(j=0;j<n2;j++){
	  ...
	  if (cabs(s1-s3)<1.0e-12*(cabs(s1)+cabs(s3))){
	    alp = (complex double*)malloc((i+1+j+1)*sizeof(complex double));
	    bet = (complex double*)malloc(1*sizeof(complex double));
	    k1 = i+1+j+1;
	    k2 = 0;
	  }else{
	    alp = (complex double*)malloc((i+1)*sizeof(complex double));
	    bet = (complex double*)malloc((j+1)*sizeof(complex double));
	    k1 = i+1;
	    k2 = j+1;
	  }
